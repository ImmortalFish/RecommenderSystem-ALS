...
def ColdStart(path):
    ...
    movies_afterhandle = sc.textFile("D:/文档/毕业设计/数据集/ml-20m/movies.csv").map(HandleMovieList).collect()
    ...
    if len(list_movie) > 10:
        ...
        for i in range(0,10):
            print("%d: %d  %s" % (i,list_movie[i],movie_data[list_movie[i]]))
...
def Recommendation(path):
    ...
    rating_compare = ReadMyRatingFile("D:/文档/毕业设计/数据集/ml-20m/test_4_last.data")
...
if __name__ == "__main__":
      ...
      user_data = sc.textFile("D:/文档/毕业设计/数据集/ml-20m/ratings_4_yiqianwan.csv").map(HandleRating)
      #movie_data是电影的数据，具体多少个电影后面也会打印出来，这里将其弄成dict主要是为了后面出推荐结果时好寻找并打印
      movie_data = dict(sc.textFile("D:/文档/毕业设计/数据集/ml-20m/movies.csv").map(HandleMovie).collect())
      
      main() 
      
      #D:\文档\毕业设计\数据集\ml-20m\test_4.data
      #D:\文档\毕业设计\数据集\my_genres.txt
      #list=[3.491101,3.486286,3.484586,3.483096,3.482925,3.483308,3.483035,3.482648,3.482919,3.481898]